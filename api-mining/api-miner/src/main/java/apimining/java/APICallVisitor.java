...
public class APICallVisitor extends JavaApproximateTypeInferencer {
	...
	/** dayen: interesting APIs that should be identified */
	private final String[] interestingAPIs;
	 ...
	 *            subfolders
	 */
	public APICallVisitor(final CompilationUnit unit, final String wildcardNameSpaceFolder, final String[] interestingAPIs) {
//		public APICallVisitor(final CompilationUnit unit, final String wildcardNameSpaceFolder) {
		...
		this.interestingAPIs = interestingAPIs;
	}
	...
	public void process() {
		...
		for (final String wimport : interestingAPIs) {
			try {
				final List<String> qNames = Files.readLines(new File(wildcardNameSpaceFolder + "api/" + wimport),
						Charsets.UTF_8);
				for (final String qName : qNames) {
					final String name = qName.substring(qName.lastIndexOf('.') + 1);
					if (Character.isLowerCase(name.charAt(0)))
						methodImports.put(name, qName.substring(0, qName.lastIndexOf('.')));
					System.out.println("+++++ INFO: Successfully resolved interesting API methods import " + wimport);
				}
			} catch (final IOException e) {
				System.out.println("***** WARNING: Interesting API import '" + wimport
						+ "' not resolved. Please supply api namespace file.");
			}
		}
...
//					return "UNRESOLVED"; // TODO could do better here?
				/** dayen: if two methods are nested and if both are in the interesting API, use the namespace's fqn */
				if (inv.getExpression().getNodeType() == ASTNode.SIMPLE_NAME)
					if (methodImports.containsKey(inv.getName().toString())) {
						if (inv.getParent().getNodeType() == ASTNode.METHOD_INVOCATION) {
							final String name = ((MethodInvocation) inv.getParent()).getName().toString();
							if (methodImports.containsKey(name))
								return methodImports.get(name); 
						}
					}
		...
		else if (expression.getNodeType() == ASTNode.SUPER_METHOD_INVOCATION) // super().method1().method2()
			return "UNRESOLVED";
...
//			throw new RuntimeException("Unhandled Expression ASTNode. Please implement.");
			System.out.println("***** WARNING: Unhandled Expression ASTNode. Please implement.");
			return "UNRESOLVED";
		}
